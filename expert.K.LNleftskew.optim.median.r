...
#12 Jan 2010
#previous version works ("expert.K.LNleftskewOLD2.R")
#changes
#1) ssLN to ss
#2) fitLN to fit
...
#Lhat <- 2000; Uhat<-5000; pihat <-.85; Mhat <- 4000; new.alpha=.95
#source("expert.K.LNleftskew.optim.R")
#expert.K.LN.results <- expert.K.LNleftskew(Lhat, Uhat, Mhat, pihat, .95)

#Lhat <- 2; Uhat<-10; pihat <-.65; Mhat <- 8; new.alpha=.95
#Lhat <- 2; Uhat<-10; pihat <-.85; Mhat <- 8; new.alpha=.95

#Lhat <- 50; Uhat<-100; pihat <-.85; Mhat <- 90; new.alpha=.95

#Lhat <- 5000; Uhat<-12000; pihat <-.85; Mhat <- 10250; new.alpha<-.95 

#####

expert.K.LNleftskew <-function(Lhat, Uhat, Mhat, pihat, new.alpha, best.type.status, ee.type){
...
ss.mean.results <-ss(fit.best.mean.mu, fit.best.mean.sig , pihat)
...
ss.median.results <-ss(fit.best.median.mu, fit.best.median.sig , pihat)
...
#lines(c(1,2),rep(ss.mode.results$mean+howskew,2), col=2,lwd=2)
#lines(c(1,2), rep(ss.mode.results$mode+howskew,2),col=4,lwd=2)#maths
lines(c(1,2), rep(Mhatold-(Mhat-ss.mode.results$mode),2),col=4,lwd=2)#maths
lines(c(1,2), rep(Mhatold,2),col=3,lwd=2)#best guess
...
lines(c(1,2),rep(Mhatold-(Mhat-ss.mean.results$mean),2), col=4,lwd=2)
...
lines(c(1,2), rep(Mhatold-(Mhat-ss.median.results$median),2), col=4,lwd=2)
lines(c(1,2), rep(Mhatold,2),col=3,lwd=2)#best guess
...
#Ksp.LN <-rlnorm(10000, fit.best.mode.mu, fit.best.mode.sig)
#flip distribution so that it is skewed left 
#see "playFlipLNtoMirrowLN.R"
#Ksp <- max(Ksp.LN)-Ksp.LN+1
#Ksp <- Uhat-Ksp.LN+1
...
ss.mean.results$mean <-Mhatold-(Mhat-ss.mean.results$mean)
ss.median.results$median <-Mhatold-(Mhat-ss.median.results$median)

feedback.mode.results$mode = Mhatold-(Mhat-feedback.mode.results$mode)
feedback.mode.results.optim$mode= Mhatold-(Mhat-feedback.mode.results.optim$mode)
